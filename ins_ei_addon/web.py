...
def validate(value):
    return validate_mapping_config({"mappings": value}).errors
...
class H(BaseHTTPRequestHandler):
        ...
        if p.endswith("/api/catalog"):
            return self.js({
                ...
                "meta": {
                    c: {
                        x: {
                            "unit": point_spec(c, x).unit,
                            "role": point_spec(c, x).role,
                            "freshness": point_spec(c, x).freshness,
                            "description": point_description(c, x),
                        }
                        for x in point_choices(c)
                    }
            ...
            return self.js([
                {
                    ...
                    "state": x.get("state"),
                    "unit": x.get("unit"),
                    "suggested_domain": x.get("suggested_domain"),
                    "suggested_point": x.get("suggested_point"),
                }
                for x in items
            ])
    ...
    def do_POST(self):
        ...
        if p.endswith("/api/discovery/accept"):
            x = body["item"]
            candidate = {"component_id": x.get("suggested_domain"), "point": x.get("suggested_point"), "entity_id": x.get("entity_id")}
            new = rows() + [candidate]
            err = validate(new)
            if err:
                return self.js({"errors": err}, 400)
            save(new)
            return self.js({"rows": new})
        if p.endswith("/api/import"):
            current = rows()
            entities = {x["entity_id"] for x in current}
            points = {f'{x["component_id"]}.{x["point"]}' for x in current}
            added = skipped = 0
            for x in _parse_bulk_text(body.get("text", "")):
                key = f'{x["component_id"]}.{x["point"]}'
                if x["entity_id"] in entities or key in points:
                    skipped += 1
                    continue
                current.append(x)
                entities.add(x["entity_id"])
                points.add(key)
                added += 1
            ...
            if err:
                return self.js({"errors": err}, 400)
            save(current)
            return self.js({"rows": current, "added": added, "skipped": skipped})
        if p.endswith("/api/mappings"):
            value = body.get("rows", [])
            err = validate(value)
            if err:
                return self.js({"errors": err}, 400)
            save(value)
            return self.js({"saved": len(value)})
