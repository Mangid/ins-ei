"""Minimal local INS-EI Ingress UI server."""
from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer
from pathlib import Path
import json
from urllib.parse import urlparse
from ins_ei.catalog import component_choices, point_choices
from ins_ei.adapters.mapping import _parse_bulk_text
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path.endswith("/api/catalog"):return self._json({"components":component_choices(),"points":{c:point_choices(c) for c in component_choices()}})
        if path.endswith("/api/mappings"):return self._json(load_rows())
    ...
    def do_POST(self):
        ...
        if path.endswith("/api/import"):
            rows=load_rows();existing_entities={r["entity_id"] for r in rows};existing_points={f'{r["component_id"]}.{r["point"]}' for r in rows};added=[];skipped=[]
            for item in _parse_bulk_text(body.get("text","")):
                logical=f'{item["component_id"]}.{item["point"]}'
                if item["entity_id"] in existing_entities or logical in existing_points:skipped.append(item);continue
                rows.append(item);added.append(item);existing_entities.add(item["entity_id"]);existing_points.add(logical)
            save_rows(rows);return self._json({"added":len(added),"skipped":len(skipped),"rows":rows})
        if path.endswith("/api/mappings"):
            rows=body.get("rows",[]);save_rows(rows);return self._json({"saved":len(rows)})
