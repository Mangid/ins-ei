...
CATALOG={
 ...
 "BATTERY":{
  "soc":PointSpec("%","STATE","OPTIMIZATION"),"power":PointSpec("W","STATE","OPTIMIZATION"),"voltage":PointSpec("V","STATE","DIAGNOSTIC"),
 ...
 "BUFFER":{"pump_state":PointSpec(None,"STATE","MONITORING"),"state":PointSpec(None,"STATE","MONITORING"),"min_temperature_off":PointSpec("°C","STATE","MONITORING"),"temperature_top":PointSpec("°C","SLOW","OPTIMIZATION"),"temperature_upper":PointSpec("°C","SLOW","OPTIMIZATION"),"temperature_middle":PointSpec("°C","SLOW","OPTIMIZATION"),"temperature_lower":PointSpec("°C","SLOW","OPTIMIZATION"),"temperature_bottom":PointSpec("°C","SLOW","OPTIMIZATION"),"volume":PointSpec("L","STATE","DIAGNOSTIC")},
 ...
 "DHW":{"one_time_charge":PointSpec(None,"STATE","MONITORING"),"temperature":PointSpec("°C","SLOW","OPTIMIZATION"),"temperature_top":PointSpec("°C","SLOW","OPTIMIZATION"),"temperature_bottom":PointSpec("°C","SLOW","OPTIMIZATION"),"target_temperature":PointSpec("°C","STATE","OPTIMIZATION"),"pump_state":PointSpec(None,"STATE","MONITORING"),"state":PointSpec(None,"STATE","MONITORING"),"volume":PointSpec("L","STATE","DIAGNOSTIC")},
 ...
 "SOLAR_THERMAL":{"collector_temperature":PointSpec("°C","NORMAL","OPTIMIZATION"),"flow_temperature":PointSpec("°C","NORMAL","OPTIMIZATION"),"return_temperature":PointSpec("°C","NORMAL","MONITORING"),"storage_temperature":PointSpec("°C","SLOW","OPTIMIZATION"),"pump_state":PointSpec(None,"STATE","MONITORING"),"flow_rate":PointSpec("l/min","NORMAL","MONITORING"),"thermal_power":PointSpec("W","FAST","OPTIMIZATION"),"energy_today":PointSpec("kWh","STATE","OPTIMIZATION"),"energy_total":PointSpec("kWh","STATE","DIAGNOSTIC")},
 "POWER_TO_HEAT":{"electrical_power":PointSpec("W","STATE","OPTIMIZATION"),"temperature":PointSpec("°C","SLOW"),"target_temperature":PointSpec("°C","STATE")},
 "HEAT_PUMP":{"electrical_power":PointSpec("W","FAST","OPTIMIZATION"),"thermal_power":PointSpec("W","STATE","OPTIMIZATION"),"flow_temperature":PointSpec("°C","NORMAL","OPTIMIZATION"),"return_temperature":PointSpec("°C","NORMAL","MONITORING"),"source_temperature":PointSpec("°C","NORMAL","MONITORING"),"compressor_state":PointSpec(None,"STATE","MONITORING"),"compressor_starts":PointSpec(None,"STATE","DIAGNOSTIC"),"runtime":PointSpec("h","STATE","DIAGNOSTIC"),"heat_energy_total":PointSpec("kWh","STATE","DIAGNOSTIC"),"defrost_state":PointSpec(None,"STATE","DIAGNOSTIC"),"pump_speed":PointSpec("%","STATE","DIAGNOSTIC"),"fan_speed":PointSpec("%","STATE","DIAGNOSTIC"),"state":PointSpec(None,"STATE","OPTIMIZATION")},
 "PELLET_BOILER":{"operating_mode":PointSpec(None,"STATE","MONITORING"),"current_power":PointSpec("W","FAST","OPTIMIZATION"),"boiler_temperature":PointSpec("°C","NORMAL","OPTIMIZATION"),"flame_temperature":PointSpec("°C","NORMAL","MONITORING"),"flow_temperature":PointSpec("°C","NORMAL","OPTIMIZATION"),"return_temperature":PointSpec("°C","NORMAL","MONITORING"),"fuel_consumption_total":PointSpec("kg","STATE","DIAGNOSTIC"),"fuel_consumption_today":PointSpec("kg","STATE","DIAGNOSTIC"),"fuel_consumption_yesterday":PointSpec("kg","STATE","DIAGNOSTIC"),"burner_starts":PointSpec(None,"STATE","DIAGNOSTIC"),"runtime":PointSpec("h","STATE","DIAGNOSTIC"),"state":PointSpec(None,"STATE","OPTIMIZATION")},
...
("BUFFER","min_temperature_off"):"Aktueller Parameter für die Mindest-/Ausschalttemperatur der Pufferladung. Bei ÖkoFEN entspricht dies pu1.mintemp_off. Wird zunächst nur überwacht; eine spätere Änderung ist nur zulässig, wenn buffer_deep_charge_allowed für die konkrete Anlage aktiviert ist.",
...
("DHW","one_time_charge"):"Einmalige Warmwasserladung aus dem verfügbaren Heiz-/Puffersystem. Bei ÖkoFEN ww1.heat_one. Zunächst nur Monitoring; Assisted-Steuerung wird separat abgesichert.",
...
("SOLAR_THERMAL","collector_temperature"):"Aktuelle Temperatur am Solarkollektor.",
("SOLAR_THERMAL","flow_temperature"):"Aktuelle Vorlauftemperatur des Solarkreises.",
("SOLAR_THERMAL","return_temperature"):"Aktuelle Rücklauftemperatur des Solarkreises.",
("SOLAR_THERMAL","storage_temperature"):"Für die Solarthermie relevante Speichertemperatur.",
("SOLAR_THERMAL","pump_state"):"Betriebszustand der Solarkreispumpe.",
("SOLAR_THERMAL","flow_rate"):"Aktueller Volumenstrom im Solarkreis.",
("SOLAR_THERMAL","thermal_power"):"Aktuell von der Solarthermie gelieferte thermische Leistung.",
("SOLAR_THERMAL","energy_today"):"Heute von der Solarthermie gelieferte Wärmemenge.",
("SOLAR_THERMAL","energy_total"):"Kumulierte Wärmemenge der Solarthermieanlage.",
...
("HEAT_PUMP","compressor_starts"):"Fortlaufende Anzahl der Verdichter- bzw. Wärmeerzeugerstarts.",
("HEAT_PUMP","runtime"):"Fortlaufende Betriebsstunden des Wärmeerzeugers.",
("HEAT_PUMP","heat_energy_total"):"Fortlaufend erzeugte Gesamtwärmemenge der Wärmepumpe.",
("HEAT_PUMP","defrost_state"):"Aktueller Abtauzustand der Wärmepumpe.",
("HEAT_PUMP","pump_speed"):"Aktuelle Pumpendrehzahl der Wärmepumpe.",
("HEAT_PUMP","fan_speed"):"Aktuelle Ventilatordrehzahl der Wärmepumpe.",
...
("PELLET_BOILER","operating_mode"):"Freigabemodus des Pelletkessels. Bei ÖkoFEN pe1.mode: 0 = Aus, 1 = Auto. Zunächst nur Monitoring; spätere Schreibzugriffe benötigen expliziten Assisted-Modus und Restore-Schutz.",
