...
def mappings_from_dict(config:dict[str,Any])->list[EntityMapping]:
    ...
    for item in _expanded_items(config):
        ...
        result.append(EntityMapping(component_id=cid,point=point,entity_id=entity,unit=item.get("unit") or (spec.unit if spec else None),role=DataRole(item.get("role",default_role)),scale=float(item.get("scale",1.0)),invert_sign=bool(item.get("invert_sign",False)),freshness=FreshnessPolicy(item.get("freshness",default_freshness)),max_age_seconds=item.get("max_age_seconds")))
    return result
