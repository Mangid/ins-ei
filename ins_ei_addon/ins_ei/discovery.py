...
class DiscoveryCandidate:
    ...
    score:int; mapped_to:str|None=None
...
def suggest(domain,hay,dc):
    ...
    if domain=="BUFFER" and dc=="temperature":
...
def discover(states:list[dict[str,Any]],mapped:dict[str,str]|None=None)->list[DiscoveryCandidate]:
    ...
    for item in states:
        ...
        if score:out.append(DiscoveryCandidate(entity,item.get("state"),name,unit,dc,attrs.get("state_class"),best,suggest(best,hay,dc),score,mapped.get(entity)))
